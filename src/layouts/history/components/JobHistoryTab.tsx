...
import {jobColumns} from "../../../project-columns";
...
const JobHistoryTab = () => {
    ...
    const doSearch = () => {
        axios.get(serviceUrls.urls.GetJobHistoryDetails, {
            params: {
            ...
            if (response.status === 200) {
                setJobHistoryData(response.data.Data.map((item) => ({
                    ...item,
    ...
    return (
        <>
            ...
            <CustomMUIGrid width={'100%'} {...serverFilter} data={jobHistoryData} columns={jobColumns}
                           loading={loading}/>
        </>
    );
};
