...
import {osColumns} from "../../../project-columns";
...
const OsHistoryTab = () => {
    ...
    const [osHistoryData, setOsHistoryData] = useState<any>([]);
    ...
    const doSearch = () => {
        axios.get(serviceUrls.urls.GetOSHistories, {
            params: {
            ...
            if (response.status === 200) {
                setOsHistoryData(response.data.Data.map((item) => ({
                    ...item,
    ...
    return (
        <>
            ...
            <CustomMUIGrid width={'100%'} {...serverFilter} data={osHistoryData} columns={osColumns}
                           loading={loading}/>
        </>
    );
};

export default OsHistoryTab;
